...
class Account extends CI_Controller {
	...
	function dashboard(){
		...
		if (!$this->session->userdata('is_logged_in')){
			// Redirect user to our login page if not already
			// logged in.
			redirect('account');
			
		}
		
		    $this->load->view('includes/db-header');
			$this->load->view('dashboard');
			$this->load->view('includes/db-footer');
	...
	function validate_creds() {
		...
		if ($query) {
			...
			// Create an array with user data for this session.
			$data = array(
		    ...
		    // Set this session with user's data from above array.
			...
			$data['bad_creds'] = "Check your email and password and try again.";
			$this->load->view('includes/header');
	...
	function logout() {
		...
		//Remove user's session and logout
		$alldata = array('first_name' => '', 'last_name' => '', 'email' => '');
		$this->session->unset_userdata($alldata);
		$this->session->sess_destroy();
		
		$data['info'] = "Successfully Logged Out.";
	...
	function create_user(){
		...
		if ($this->form_validation->run() == FALSE) { // fails to validate.
			...
			if ($query = $this->membership_model->create_member()){
			    ...
			    $data['info'] = "Your account has been created.<br /><br />You may now login.";
